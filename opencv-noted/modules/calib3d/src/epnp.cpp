...
namespace cv
{
  ...
  void epnp::choose_control_points(void)
  {
    ...
    for (int i = 1; i < 4; i++)
    {
      double k = sqrt(dc[i - 1] / number_of_correspondences); // sqrt(eigen values)
      for (int j = 0; j < 3; j++)
      {
  ...
  void epnp::compute_barycentric_coordinates(void)
  {
    ...
    double *ci = cc_inv;
    // calculate new coordinate in this new set of basis
    for (int i = 0; i < number_of_correspondences; i++)
    {
  ...
  // (out M, in row, in new coordinate, in u, in v): fill 2 rows
  void epnp::fill_M(CvMat *M, const int row, const double *as, const double u, const double v)
  {
    // db is first position of data; 12 is num of column
    double *M1 = M->data.db + row * 12; // u(3) multiply 4 control points
    double *M2 = M1 + 12;               // v(3) multiply 4 control points
  ...
  void epnp::compute_pose(Mat &R, Mat &t)
  {
    ...
    // get coordinates of pw in the set of basis from control points. I will call it "new coordinate"
    compute_barycentric_coordinates();

    // project new coordinate(with control points) to camera
