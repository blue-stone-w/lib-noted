...
namespace cv
{
    ...
    {
        int incr = end > start ? 1 : -1; // to right or to left
        // prepare first triangle
        int pprev = start, pcur = pprev + incr, pnext = pcur + incr;
        int stacksize = 3;
        // if start and end are the same points
        if (start == end ||
        ...
        while (pnext != end)
        {
            ...
            if (CV_SIGN(by) != nsign) // compare next y and cur y
            {
    ...
    void convexHull(InputArray _points, OutputArray _hull, bool clockwise, bool returnPoints)
    {
        ...
        int i, total = points.checkVector(2); // num of points if channel is 2; or -1
        ...
        Point **pointer = _pointer.data(); // 2d pointer
        Point2f **pointerf = (Point2f **)pointer;
        Point *data0 = points.ptr<Point>(); //???
        int *stack = _stack.data();
        int *hullbuf = _hullbuf.data();

        CV_Assert(points.isContinuous()); // 如果矩阵元素在每行末尾连续存储而没有间隙，则方法返回true。 否则，它返回false

        for (i = 0; i < total; i++)
        {
            pointer[i] = &data0[i]; // share memory
        }

        // sort the point set by x-coordinate, find min and max y
        if (!is_float)
        {
            // sort points in this address range
        ...
        // first point and last point are the same point
        ...
        {
            // upper half
            int *tl_stack = stack; // head
            int tl_count = !is_float ? Sklansky_<int, int64>(pointer, 0, maxy_ind, tl_stack, -1, 1)
                                     : Sklansky_<float, double>(pointerf, 0, maxy_ind, tl_stack, -1, 1);
            int *tr_stack = stack + tl_count; // end
            int tr_count = !is_float ? Sklansky_<int, int64>(pointer, total - 1, maxy_ind, tr_stack, -1, -1)
            ...
            int stop_idx = tr_count > 2 ? tr_stack[1] : tl_count > 2 ? tl_stack[tl_count - 2]
                                                                     : -1;

            // lower half
            int *bl_stack = stack;
            int bl_count = !is_float ? Sklansky_<int, int64>(pointer, 0, miny_ind, bl_stack, 1, -1)
                                     : Sklansky_<float, double>(pointerf, 0, miny_ind, bl_stack, 1, -1);
            int *br_stack = stack + bl_count;
            int br_count = !is_float ? Sklansky_<int, int64>(pointer, total - 1, miny_ind, br_stack, 1, 1)
            ...
            if (stop_idx >= 0)
            {
                int check_idx = bl_count > 2 ? bl_stack[1] : bl_count + br_count > 2 ? br_stack[2 - bl_count]
                                                                                     : -1;
            ...
            // try to make the convex hull indices form an ascending or descending sequence
            // by the cyclic shift of the output sequence.
            if (nout >= 3)
            {
        ...
        } // endelse: first point and last point aren't the same point
    ...
    bool isContourConvex(InputArray _contour)
    {
        ...
        return depth == CV_32S ? isContourConvex_(contour.ptr<Point>(), total) : isContourConvex_(contour.ptr<Point2f>(), total);
    }
