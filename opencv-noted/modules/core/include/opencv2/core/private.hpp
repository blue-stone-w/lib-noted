...
namespace cv
{
    ...
    //! Allocate memory buffers which will not be freed, ease filtering memcheck issues. Uses fastMalloc() call
    template <typename T>
    static inline T *allocSingleton(size_t count = 1) { return static_cast<T *>(allocSingletonBuffer(sizeof(T) * count)); }
    ...
    //! Allocate memory buffers which will not be freed, ease filtering memcheck issues.  Uses generic malloc() call.
    template <typename T>
    static inline T *allocSingletonNew() { return new (allocSingletonNewBuffer(sizeof(T))) T(); }
...
{
    return borderTypeNI == cv::BORDER_CONSTANT ? ippBorderConst : borderTypeNI == cv::BORDER_TRANSPARENT ? ippBorderTransp
...
{
    return (kx == 1 && ky == 3) ? ippMskSize1x3 : (kx == 1 && ky == 5) ? ippMskSize1x5
...
static inline IppDataType ippiGetDataType(int depth)
{
    ...
    return depth == CV_8U ? ipp8u : depth == CV_8S ? ipp8s
                                : depth == CV_16U  ? ipp16u
                                : depth == CV_16S  ? ipp16s
                                : depth == CV_32S  ? ipp32s
                                : depth == CV_32F  ? ipp32f
...
{
    return (dx == 1 && dy == 0) ? ((nvert) ? iwiDerivNVerFirst : iwiDerivVerFirst) : (dx == 0 && dy == 1) ? iwiDerivHorFirst
...
#define CV_INSTRUMENT_MARK_META(IMPL, NAME, ...)                                                                                 \
    {                                                                                                                            \
        ::cv::instr::IntrumentationRegion __instr_mark__(NAME, __FILE__, __LINE__, NULL, false, ::cv::instr::TYPE_MARKER, IMPL); \
    }
